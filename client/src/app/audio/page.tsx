...
export default function AudioInput() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!audioFile && !audioURL) {
      setStatus('Please upload an audio file or record one.');
      return;
    }
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 px-5">
      <div className="text-center">
        <h1 className="text-4xl md:text-5xl font-bold mb-6 text-[#0e312d]">Upload or Record an Audio File</h1>
        <h2 className="text-base md:text-lg text-gray-600 mb-8">Please upload your audio file below or record one directly.</h2>
        <form onSubmit={handleSubmit} className="space-y-4 max-w-md md:max-w-lg mx-auto">
          ...
          <div className='text-sm text-gray-500'>Or record your own audio</div>
          <div className="flex items-center justify-center pb-2">
            <Button
              type="button"
              onClick={isRecording ? stopRecording : startRecording}
              className={`w-12 h-12 rounded-full flex items-center justify-center text-white ${isRecording ? 'bg-red-400 hover:bg-red-500' : 'bg-black hover:bg-slate-800'}`}
            >
              <FontAwesomeIcon icon={isRecording ? faStop : faMicrophone} className="text-2xl" />
            </Button>
          </div>
          {audioURL && (
            <div className="mt-4">
